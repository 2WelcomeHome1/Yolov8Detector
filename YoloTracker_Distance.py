...
class PeopleDetector():
    ...
    def dist(self, p1, p2):
        import math
        return math.hypot(p2[0] - p1[0], p2[1] - p1[1])#*0.0002645833
    ...
    def _create_detection_info(self, obj_id, x1, y1, x2, y2, class_id, score, result):
        ...
        for pt in range(l):
            if not pt + 1 == l: 
                self.frame = cv2.line(self.frame, self.dic[str(obj_id)][0][pt], self.dic[str(obj_id)][0][pt+1], color, 2)
                self.distance.append(self.dist(self.dic[str(obj_id)][0][pt], self.dic[str(obj_id)][0][pt+1]))
            self.dic[str(obj_id)][2][0] = sum(self.distance)
        
        text = "ID: %s.%s %s, Dist: %s" % (obj_id, result.names[class_id], round(score,2), round(self.dic[str(obj_id)][2][0],2))
